...
use rspack_core::{
  ...
  SideEffectOption, SourceType, UsedExportsOption, Compilation
};
...
use crate::RawOptimizationOptions;
...
fn get_modules_size(modules: &[&dyn Module], compilation: &Compilation) -> f64 {
  let mut size = 0f64;
...
fn get_plugin_options(
  ...
  let cache_groups = vec![
    CacheGroup {
      ...
      priority: 40.0,
      test: CacheGroupTest::Fn(Arc::new(move |ctx: CacheGroupTestFnCtx| {
        Ok(Some(ctx.module.name_for_condition().map_or(false, |name| {
          strategy
      ...
      used_exports: true,
      layer: rspack_plugin_split_chunks::create_default_module_layer_filter(),
      max_initial_requests: 25.0,
      max_async_requests: 25.0,
    ...
    CacheGroup {
      ...
      name: ChunkNameGetter::Fn(Arc::new(move |ctx| {
        ...
        let mut hash_digest: Option<RspackHashDigest> = None;
        match ctx.module.module_type() {
          ModuleType::Css | ModuleType::CssModule | ModuleType::CssAuto => {
            if ctx.module.build_info().is_some() {
              hash_digest = ctx.module.build_info().unwrap().hash.clone();
            } else {
        ...
        Ok(Some(if hash_digest.is_some() {
          hash_digest.unwrap().rendered(8).to_string()
        } else {
      ...
      test: CacheGroupTest::Fn(Arc::new(move |ctx| {
        Ok(Some(
          ctx
            ...
            && get_modules_size(&[ctx.module], ctx.compilation) > 160000.0,
        ))
      })),
      priority: 30.0,
      min_chunks: 1,
